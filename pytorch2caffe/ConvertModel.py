...
def ConvertModel_caffe(pytorch_net, InputShape, model_type, softmax=False):
    ...
    pytorch_net.eval()
    global inputs
    if model_type == 'dispnet':
        n, c, h, w = InputShape
        inputs = Variable(torch.rand(n, c, h, w), requires_grad=True)
        outputs = pytorch_net(inputs)
    elif model_type == 'posenet':
        # target_input = Variable(torch.rand(InputShape[0]), requires_grad=True)
        # ref_inputs = [Variable(torch.rand(shape), requires_grad=True) for shape in InputShape[1:]]
        #  inputs = [target_input, ref_inputs]
        n, c, h, w = InputShape
        inputs = Variable(torch.rand(n, c, h, w), requires_grad=True)
        _, outputs = pytorch_net(inputs)      
